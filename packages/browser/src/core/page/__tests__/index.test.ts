...
describe(getDefaultPageContext, () => {
  describe('hash', () => {
    it('strips the hash from the URL', () => {
      window.location.href = 'http://www.cloudflare.local#test'
      const defs = getDefaultPageContext()
      expect(defs.url).toBe('http://www.cloudflare.local')

      window.location.href = 'http://www.cloudflare.local/#test'
      const defs2 = getDefaultPageContext()
      expect(defs2.url).toBe('http://www.cloudflare.local/')
    })
  })
  ...
  describe('canonical URL', () => {
    ...
    it('handles canonical links', () => {
      el.setAttribute('href', 'http://www.cloudflare.local')
      document.body.appendChild(el)
      const defs = getDefaultPageContext()
      expect(defs.url).toEqual('http://www.cloudflare.local')
    })

    it('favors canonical path over location.pathname', () => {
      window.location.pathname = '/nope'
      el.setAttribute('href', 'http://www.cloudflare.local/test')
      document.body.appendChild(el)
      const defs = getDefaultPageContext()
    ...
    it('handles canonical links with a path', () => {
      ...
      document.body.appendChild(el)
      const defs = getDefaultPageContext()
      expect(defs.url).toEqual('http://www.cloudflare.local/test')
      expect(defs.path).toEqual('/test')
    })

    it('handles canonical links with search params in the url', () => {
      el.setAttribute('href', 'http://www.cloudflare.local?test=true')
      document.body.appendChild(el)
      const defs = getDefaultPageContext()
      expect(defs.url).toEqual('http://www.cloudflare.local?test=true')
    })
    ...
    it('will add search params from the document to the canonical path if it does not have search params', () => {
      ...
      document.body.appendChild(el)
      const defs = getDefaultPageContext()
      expect(defs.url).toEqual('http://www.cloudflare.local?foo=123')
    })
  })
})
