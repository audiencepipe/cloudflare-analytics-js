...
describe('user', () => {
  ...
  describe('()', () => {
    ...
    it('should not pick the old "_sio" if anonymous id is present', () => {
      ...
      jar.set('cfjs_anonymous_id', 'new-anonymous-id')
    ...
    it('should create anonymous id if missing (persist: default (true))', () => {
      ...
      expect(jar.get('cfjs_anonymous_id')?.length).toBe(36)
      expect(localStorage.getItem('cfjs_anonymous_id')?.length).toBe(38)
    })
    ...
    it('should create anonymous id if missing (persist: false)', () => {
      ...
      assert(user.anonymousId()?.length === 36)
      expect(jar.get('cfjs_anonymous_id')).toBeUndefined()
      expect(localStorage.getItem('cfjs_anonymous_id')).toBeNull()
      expect(ignoreProbeCookieWrites(setCookieSpy).length).toBe(0)
    })

    it('should not overwrite anonymous id', () => {
      jar.set('cfjs_anonymous_id', 'anonymous')
  ...
  describe('#anonymousId', () => {
    ...
    describe('when cookies are disabled', () => {
      ...
      it('should get an id from the store', () => {
        store.set('cfjs_anonymous_id', 'anon-id')
        expect(user.anonymousId()).toEqual('anon-id')
      })

      it('should set an id to the store', () => {
        user.anonymousId('anon-id')
        assert(store.get('cfjs_anonymous_id') === 'anon-id')
      })
      ...
      it('should get an id without quotes from the store', () => {
        window.localStorage.setItem('cfjs_anonymous_id', 'abc-def')
    ...
    describe('when cookies are enabled', () => {
      it('should get an id from the cookie', () => {
        jar.set('cfjs_anonymous_id', 'anon-id')
        assert(user.anonymousId() === 'anon-id')
      })

      it('should set an id to the cookie', () => {
        user.anonymousId('anon-id')
        assert(jar.get('cfjs_anonymous_id') === 'anon-id')
      })

      it('should set anonymousId in both cookie and localStorage', () => {
        user.anonymousId('anon0')
        assert.equal(jar.get('cfjs_anonymous_id'), 'anon0')
        assert.equal(store.get('cfjs_anonymous_id'), 'anon0')
      })
      ...
      it('should not set anonymousId in localStorage when localStorage fallback is disabled', () => {
        ...
        user.anonymousId('anon0')
        assert.equal(jar.get('cfjs_anonymous_id'), 'anon0')
        assert.equal(store.get('cfjs_anonymous_id'), null)
      })

      it('should copy value from cookie to localStorage', () => {
        user = new User()
        jar.set('cfjs_anonymous_id', 'anon1')

        assert.equal(user.anonymousId(), 'anon1')
        assert.equal(store.get('cfjs_anonymous_id'), 'anon1')
      })
      ...
      it('should fall back to localStorage when cookie is not set', () => {
        ...
        assert.equal(jar.get('cfjs_anonymous_id'), 'anon12')

        // delete the cookie
        jar.remove('cfjs_anonymous_id')
        assert.equal(jar.get('cfjs_anonymous_id'), null)
      ...
      it('should write to both cookie and localStorage when generating a new anonymousId', () => {
        ...
        assert.equal(jar.get('cfjs_anonymous_id'), anonId)
        assert.equal(store.get('cfjs_anonymous_id'), anonId)
      })
  ...
  describe('storage', () => {
    it('allows custom storage priority', () => {
      ...
      // Set a cookie first
      jar.set('cfjs_anonymous_id', expected)
      store.set('cfjs_anonymous_id', 'localStorageValue')
      const user = new User({
        storage: {
    ...
    it('custom storage priority respects availability', () => {
      ...
      // Set a cookie first
      jar.set('cfjs_anonymous_id', 'CookieValue')
      disableCookies()
      store.set('cfjs_anonymous_id', expected)
      const user = new User({
        storage: {
          stores: [StoreType.Cookie, StoreType.LocalStorage, StoreType.Memory],
        },
    ...
    it('persist option overrides any custom storage', () => {
      ...
      expect(jar.get('cfjs_user_id')).toBeFalsy()
      expect(store.get('cfjs_user_id')).toBeFalsy()
...
describe('group', () => {
  ...
  it('id() should fallback to localStorage', function () {
    ...
    jar.remove('cfjs_group_id')

    assert.equal(jar.get('cfjs_group_id'), null)
    assert.equal(group.id(), 'gid')
    assert.equal(store.get('cfjs_group_id'), 'gid')
  })
  ...
  it('id() should not persist when persist disabled', () => {
    ...
    expect(group.id()).toBe('gid')
    expect(jar.get('cfjs_group_id')).toBeFalsy()
    expect(store.get('cfjs_group_id')).toBeFalsy()
    expect(ignoreProbeCookieWrites(setCookieSpy).length).toBe(0)
  })
...
describe('Custom cookie params', () => {
  it('allows for overriding keys', () => {
    ...
    expect(document.cookie).toMatchInlineSnapshot(`"cfjs_user_id=some_id"`)
